...
local cur prev words cword
    ...
    if [[ -z $cmd ]]; then
        _fzf_handle_dynamic_completion kubectl "$@"
        return
    fi
...
if type __start_kubectl >/dev/null 2>&1 && ! type _kubectl >/dev/null 2>&1; then
    export _fzf_orig_completion_kubectl=__start_kubectl
    complete -F _fzf_complete_kubectl -o default -o bashdefault kubectl
else
export _fzf_orig_completion_kubectl=_kubectl
complete -F _fzf_complete_kubectl -o default -o bashdefault kubectl
fi
